...
export function Approvance({ approvance, amount, duration, account }) {
    ...
    if(approvance){
        ...
        return <div onClick={() => approve(account, library)} className="summary__button button">{approveloader ? <ButtonLoader /> : "Approve"}</div>
    }
    else {
...
export function Lock({ approvance, duration, account}){
    ...
    if(approvance && agreement && amount >= 1500){
        return (
            <div onClick={() => stake(amount, (duration === 1 ? 12 : duration), account, history, library)} className={ !lockloader ? "summary__button button" : "summary__button loading button"}>
