...
export default function Widget({tokens}) {
    ...
    const swapToken = (side) => {
        if (tokens.length > 1) {
            if (side === 'next') {
                ...
                dispatch(updateIndex(currentToken - 1))
                if (currentToken === 0) {
                    dispatch(updateIndex(tokens.length - 1))
                }
            }
        }
    }
    ...
    return (
        <div className="nft__widget">
            <div
                style={x === 0 ? cannotClickStyle : {}}
                onClick={(item) => swiperHandler('prev')}
                className={!withdrawed ? 'left-arrow arrow' : 'left-arrow arrow--disabled'}>
            ...
            <div
                style={Math.abs(x) === rowLength ? cannotClickStyle : {}}
                onClick={(item) => swiperHandler('next')}
                className={!withdrawed ? 'right-arrow arrow' : 'right-arrow arrow--disabled'}>
